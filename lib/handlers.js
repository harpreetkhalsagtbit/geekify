...
let funcDiff  = (res) => {
	return (prevValue) => {
		return geekExecuteCommand(`git diff  ${prevValue.prev || prevValue.current.trim()} ${prevValue.current.trim()} --color=always`)
	}
}
